                            ...
                            float minValue, ScalarLoc* points, int &numTriangles)
{
    ...
    ScalarLoc verts[8];                //vertices of a starting cube
    Vector3D intVerts[12];            //linearly interpolated vertices on each edge
    ...
    for(int n=0; n < numCubes; n++) {
        ...
        verts[0] = ScalarLoc(points[ind]);
        verts[1] = ScalarLoc(points[ind + YtimeZ]);
        verts[2] = ScalarLoc(points[ind + YtimeZ + 1]);
        verts[3] = ScalarLoc(points[ind + 1]);
        verts[4] = ScalarLoc(points[ind + pointsZ]);
        verts[5] = ScalarLoc(points[ind + YtimeZ + pointsZ]);
        verts[6] = ScalarLoc(points[ind + YtimeZ + pointsZ + 1]);
        verts[7] = ScalarLoc(points[ind + pointsZ + 1]);
                        ...
                        Vector4D gradVerts[8], Vector3D grads[12], int &indGrad)
{
    ...
    Vector3D factor(1.0/(2.0*gradFactorX), 1.0/(2.0*gradFactorY), 1.0/(2.0*gradFactorZ));
    //get the index
    int cubeIndex = int(0);
    for(int n=0; n < 8; n++) if(verts[n].value <= minValue) cubeIndex |= (1 << n);
    //check if its completely inside or outside
...
float* MCFind(int ncellsX, int ncellsY, int ncellsZ, float minValue, ScalarLoc* points)
{
    ...
    ScalarLoc* verts[8];        //store address of each point rather than copying x,y,z, and val
